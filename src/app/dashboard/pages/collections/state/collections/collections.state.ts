...
export class CollectionsState {
	...
	constructor(private nodeClientService: NodeClientService, private store: Store) {}
	...
	static getCollectionsByIds(collectionIds: string[]) {
		return createSelector([CollectionsState], ({ collections }: CollectionsStateModel) => {
	...
	setCollectionsByIds(
		...
		const collectionSet = Array.isArray(collections) ? collections : [collections];

		setState(
			patch({
				collections: collectionSet.reduce((acc, value) => ({
					...acc,
					[value.id]: value
				}), {})
			})
